...
│  组件库 (Button/Card/Modal) │ 图表(wx-charts)   │
├──────────────────────────────────────────────────┤
│                   微信能力层                      │
...
| 分包 | 页面 | 预估大小 |
|------|------|----------|
| **主包** | 登录页、Chat、Profile、首页Dashboard | ≤ 2MB |
| **A (训练)** | Plans列表、PlanDetail、PlanExecute、PlanGenerate | ≤ 2MB |
| **B (记录)** | History、Trends、Measurements、Calendar | ≤ 2MB |
| **C (内容)** | Exercises列表、ExerciseDetail、Muscles、MuscleDetail、Badges、Gallery | ≤ 2MB |
...
| Chat | pages/chat/index | AI 对话，训练/围度记录 |
| Profile | pages/profile/index | 用户信息，统计，快捷入口 |
...
| 页面 | 路径 | 功能 |
|------|------|------|
| 动作库 | pages/exercises/index | 动作列表，肌肉筛选 |
...
| 图表 | 用途 | 库 |
|------|------|-----|
| 折线图 | 围度趋势、体重趋势 | wx-charts Line |
| 柱状图 | 每周训练次数 | wx-charts Column |
| 饼图 | 肌肉群训练容量占比 | wx-charts Pie |
| 环形图 | 计划完成进度 | wx-charts Ring |
...
| 端点 | 方法 | 说明 |
|------|------|------|
| /api/auth/wechat | POST | 微信 code 换 token |
